...
public class ElementStatus implements Status{
	...
	public Collection<Attribute> getAttributes() throws Exception {
		...
		Attribute attr;
		attr = new StringAttribute(null,"basis state",0,AttrSysType.NORMAL);
		try {
		...
		attrs.add(attr);
		attr = new IntegerAttribute(null,"element state",0,AttrSysType.NORMAL);
		try {
		...
		attrs.add(attr);
		attr = new IntegerAttribute(null,"ip_evt_cnt",0,AttrSysType.NORMAL);
		try {
